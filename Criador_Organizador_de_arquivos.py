...
print('Inicinado o Script de Criação e Organização de Arquivos \n')
...
# criando a pasta e subpastas para depois usar nos arquivos
print('Iniciando a Criação de Pastas e Subpastas \n')
pasta_principal = os.path.join(diretorio_projeto,'arquivos_para_organizar')
...
# Lista de extensões para a criação dos arquivos 
extensoes = ['.txt', '.pdf', '.jpeg', '.png', '.docx', '.csv', '.mp3', '.mp4']

# Loop para a criação dos arquivos, ainda não organizados em cada pasta
print('\nIniciando a criação de arquivos \n')
for i in range(35):
    # variavel responsavel por "Escolher a estensão"
    ext = random.choice(extensoes)
    # Variavel responsavel por "Criar" o nome do arquivo
    nome = f'{''.join(random.choices(string.ascii_lowercase, k = 8))}{ext}'
    #Variavel responsavel por formar o caminho para onde os arquivos gerados vão 
    caminho_arquivo = os.path.join(pasta_principal, nome)
    print(f'Criando o arquivo: {nome}...')

    # Responsavel por abrir as pastas, no caso abrir a pasta principal e colocar o arquivo gerado a cima dentro dela 
